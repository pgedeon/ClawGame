...
| T2 | Topdown | "Repaint the walls" (`sprite.color` → `#7c3aed`) | **FAILED** → **VERIFIED 2026-08-26** (see §7.1) | *2026-08-25:* All 7 wall entities render `0x64748b` in BOTH passes — same `getColorForType` override as P2 (walls type-map to `obstacle`). Note this also means the template's own wall color `#57534e` never displays today; walls are always obstacle-grey in preview. |
...
### §7.1 Re-verification after tint passthrough — P2 + T2 (2026-08-26)

The root cause of the P2/T2 failures above was fixed by `feat/sprite-tint` (`f415b32`, merged to main via `87a1c55`): `ClawgamePhaserScene.createEntity` now parses bootstrap `entity.tint` (carried from `sprite.color`) and uses it as the rectangle fill color / asset `setTint`, falling back to the type map only for absent/malformed colors; the phaser-html export generator mirrors the rule (parity). With that landed, the two deferred recipes were re-verified end-to-end through the **real UI chain** (not the probe harness): landing-page one-click launch → GamePreviewPage → FirstRunEditCard chip → mock-AI recipe mutation → diff Apply (`api.writeFile`) → scene remount → **Start Game (Play mode, the ruling #4 bar)**.

Method: headless-Chromium harness (`tmp-verify-recolor.mjs`, untracked), two fresh projects per recipe via the production creation path — project A captured at Play unmutated (baseline), project B captured at Play after chip+Apply. Evidence per pass: canvas screenshot decoded in-page with exact-color pixel counts (±3/channel), disk `scenes/main-scene.json` before/after, console/pageerror log. Zero page errors, zero console errors across all four loads.

| Recipe | Baseline at Play | Mutated at Play | Disk mutation |
|---|---|---|---|
| P2 "Make the player red" | blue `#3b82f6` player: **1504 px**; red `#ef4444` (template enemy): 961 px | red: **3010 px** (3.1× baseline); blue: **0 px** (was 1504) | `player-1` `#3b82f6` → `#ef4444`; all other entities byte-identical |
| T2 "Repaint the walls" | wall grey `#57534e`: **92405 px**; purple `#7c3aed`: **0 px** | purple: **126670 px**; grey: **0 px** | all 7 `wall-*` entities → `#7c3aed`; enemies/powerup/chest untouched |

Screenshots confirm visually: blue→red player (platformer), grey→purple walls (topdown), no overlays obscuring the canvas at capture time. Bonus evidence that the tint passthrough is live in the served bundle: baseline topdown walls now render their own `#57534e` (pre-fix they rendered type-map `0x64748b`, which is also why the slice-2c baseline showed obstacle-grey). Raw data `/tmp/recipes-recolor-verify-v2/results.json`.

Consequence: P2 and T2 graduate from DEFERRED to shippable chips in `packages/shared/src/recipes.ts` (same commit as this section). P1 (pickup→score loop) and P3 (movingPlatform consumer) remain DEFERRED — no runtime consumer yet.

— builder lane A (recipes-recolor re-verification), 2026-08-26
