...
describe('preview runtime config', () => {
  it('defaults to the legacy canvas runtime when no preference exists', () => {
    const storage = { getItem: vi.fn(() => null) };

    expect(getRequestedPreviewRuntimeKind(storage)).toBe(DEFAULT_PREVIEW_RUNTIME_KIND);
  });

  it('falls back to legacy canvas when phaser4 is requested', () => {
    const storage = { getItem: vi.fn(() => 'phaser4') };
    ...
    expect(selection.reason).toContain('not mounted');
  });

  it('keeps the legacy canvas runtime active when explicitly requested', () => {
    const storage = { getItem: vi.fn(() => 'legacy-canvas') };
