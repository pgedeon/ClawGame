...
describe('previewReplayState', () => {
  ...
  it('restores preview runtime collections and stats from a snapshot', () => {
    ...
    const towers = [{ id: 'old-tower', x: 0, y: 0, range: 50, damage: 1, fireRate: 1000, lastShot: 0, color: '#000' }];
    const tdState = {
      waveIndex: 0,
      waveTimer: 0,
      spawnQueue: [],
    ...
    const restored = restorePreviewReplayState(
      {
        ...
        runtime: {
          ...
          towers: [{ id: 'tower-1', x: 8, y: 9, range: 150, damage: 20, fireRate: 800, lastShot: 200, color: '#abc' }],
          tdState: {
            waveIndex: 2,
            waveTimer: 300,
            spawnQueue: [],
    ...
    expect(towers).toEqual([{ id: 'tower-1', x: 8, y: 9, range: 150, damage: 20, fireRate: 800, lastShot: 200, color: '#abc' }]);
