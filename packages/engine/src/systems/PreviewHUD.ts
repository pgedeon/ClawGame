...
export class PreviewHUD {
 ...
 private renderTowerDefense(td: HUDTowerDefenseStats): void {
   ...
   // Wave countdown (large, prominent display)
   if (td.waveCountdown >= 0) {
     const countdownText = `Next wave in ${td.waveCountdown.toFixed(1)}s`;
     const boxW = 220;
     const boxH = 36;
     const boxX = this.config.width / 2 - boxW / 2;
     const boxY = 14;
     ...
     ctx.textAlign = 'center';
     ctx.fillText(countdownText, this.config.width / 2, boxY + 24);
     ctx.textAlign = 'left';
   }
   // Wave announcement
   if (td.waveMessage && td.waveMessageAlpha && td.waveMessageAlpha > 0) {
     ...
     ctx.textAlign = 'center';
     ctx.fillText(td.waveMessage, this.config.width / 2, 60);
     ctx.textAlign = 'left';
   }
 }
