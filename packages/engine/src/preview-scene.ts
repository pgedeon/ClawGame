import type { EntityType, SerializableEntity, SerializableScene } from './types';

/**
 * Scene normalization shared by the web game preview and the API phaser-html
 * export path. Both pipelines must run raw project/template JSON through
 * `normalizePreviewScene` before rendering/compiling so per-entity runtime
 * types are inferred identically on both sides (docs/export-parity.md gap 1).
 *
 * Moved here from apps/web/src/utils/previewScene.ts so @clawgame/api can
 * consume it without a cross-app relative import; the web module re-exports
